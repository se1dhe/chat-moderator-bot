  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800/60/60/60/60 rounded-2xl p-4 mb-4 shadow-sm w-full flex flex-col gap-3">
        ...
        <input className="w-full px-4 py-3 bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-neutral-800/60/60/60/60 rounded-xl focus:outline-none focus:border-primary/50 transition-colors font-medium text-[15px]" value={word} onChange={(e) => setWord(e.target.value)} placeholder={t("triggers.phrase.ph")} />
        <textarea className="w-full px-4 py-3 bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-neutral-800/60/60/60/60 rounded-xl focus:outline-none focus:border-primary/50 transition-colors font-medium text-[15px] min-h-[80px] resize-y" value={reply} onChange={(e) => setReply(e.target.value)} placeholder={t("triggers.reply.ph")}></textarea>
        
        <Row title={t("triggers.regex")}>
          <Toggle checked={isRegex} onChange={setIsRegex} />
        </Row>
        
        <button className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white shadow-[0_4px_20px_-4px_rgba(220,38,38,0.5)] rounded-xl font-bold transition-all active:scale-95  text-[15px]" onClick={handleCreate} disabled={!word || !reply}>
          {t("triggers.btn.add")}
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800/60/60/60/60 rounded-2xl p-2 mb-4 shadow-sm w-full">
        {triggers.length === 0 ? (
          ...
          triggers.map(trig => (
            <div key={trig.id} className="p-4 flex flex-col gap-2 border-b border-neutral-200 dark:border-neutral-800/60/60/60/60/50 last:border-0">
