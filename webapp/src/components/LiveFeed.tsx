...
export function LiveFeed({ standalone = false }: { standalone?: boolean }) {
  ...
  return (
    <div className={`relative overflow-hidden w-full ${standalone ? 'h-[400px]' : 'h-[360px] bg-white dark:bg-[#0a0a0a] rounded-2xl border border-neutral-200 dark:border-neutral-800/60'}`}>
      <div className="absolute top-0 left-0 right-0 p-3 bg-white/90 dark:bg-neutral-900/90 backdrop-blur-md z-10 border-b border-neutral-200 dark:border-neutral-800/60 flex items-center gap-2 text-[13px] font-bold text-neutral-500 uppercase tracking-wider">
        ...
        <AnimatePresence>
          {events.map((ev) => (
            <motion.div
              ...
              className="flex items-start gap-3 p-3 bg-neutral-50 dark:bg-black border border-neutral-200 dark:border-neutral-800/60 rounded-xl"
            >
              <div className="mt-0.5 shrink-0 bg-white dark:bg-[#0a0a0a] p-2 rounded-full border border-neutral-200 dark:border-neutral-800/60 shadow-sm">
                {getIcon(ev.action)}
              </div>
