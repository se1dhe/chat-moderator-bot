...
export function WhatsNewModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-tg-bg flex flex-col animate-in fade-in duration-300">
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
        
        {/* Background glow effects */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-tg-hint opacity-[0.03] rounded-full blur-3xl pointer-events-none"></div>
        ...
        <h2 className="text-3xl font-bold mb-4 text-tg-text tracking-tight">
          {current.title}
        </h2>
        
        <p className="text-tg-hint mb-8 text-base leading-relaxed max-w-sm">
          {current.desc}
        </p>
        ...
        <div className="flex gap-2 mb-12">
          {slides.map((_, i) => (
            <div 
              key={i} 
              className={`h-1.5 rounded-full transition-all duration-300 ${i === slide ? 'w-8 bg-red-500' : 'w-2 bg-tg-hint opacity-30'}`}
            />
          ))}
        </div>
      </div>

      <div className="p-6 bg-tg-bg pb-[max(env(safe-area-inset-bottom),1.5rem)] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <div className="flex gap-3">
          {slide > 0 && (
            <button className="btn flex-1 py-4 text-base font-semibold rounded-xl bg-transparent border border-[var(--border)] text-tg-text" onClick={() => { haptic('light'); setSlide(s => s - 1) }}>
              {t('wn.btn.back')}
            </button>
          )}
